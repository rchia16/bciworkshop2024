...
def dtw(prototype, sample, return_flag=RETURN_VALUE, 
        slope_constraint="asymmetric", window=None):
    ...
    for i in range(p):
        ...
        cost[i,start:end]=np.linalg.norm(sample[start:end] - prototype[i], 
                                         axis=1)
...
def shape_dtw(prototype, sample, return_flag=RETURN_VALUE, 
              slope_constraint="asymmetric", window=None, descr_ratio=0.05):
...
def magnitude_warp(x, sigma=0.2, knot=4):
    ...
    warp_steps = (np.ones((x.shape[2],1))*(np.linspace(0, x.shape[1]-1.,
                                                       num=knot+2))).T
    ret = np.zeros_like(x)
    for i, pat in enumerate(x):
        warper = np.array(
...
def spawner(x, labels, sigma=0.05, verbose=0):
    ...
    for i, pat in enumerate(tqdm(x)):
        ...
        if choices.size > 0:     
            ...
            path1 = dtw.dtw(
                pat[:random_points[i]], random_sample[:random_points[i]],
                dtw.RETURN_PATH,
                slope_constraint="symmetric", window=window
            )
            path2 = dtw.dtw(
                pat[random_points[i]:], random_sample[random_points[i]:], dtw.RETURN_PATH, slope_constraint="symmetric", window=window)
